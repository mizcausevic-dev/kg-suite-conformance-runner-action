...
import { REQUIRED_BLOCKS } from "./types.js";
const ALL_PROTOCOLS = [
    "agent-cards-spec",
    "mcp-tool-card-spec",
...
export function detectProtocol(input) {
    ...
    if (acv)
        return { protocol: "agent-cards-spec", version: acv };
    const tcv = asString(o.tool_card_version);
    if (tcv)
        return { protocol: "mcp-tool-card-spec", version: tcv };
    const ppv = asString(o.provenance_version);
    if (ppv)
        return { protocol: "prompt-provenance-spec", version: ppv };
    const ebv = asString(o.evidence_bundle_version);
    if (ebv)
        return { protocol: "evidence-bundle-spec", version: ebv };
    if (Array.isArray(o.resourceSpans))
        return { protocol: "otel-genai-otlp" };
    if (Array.isArray(o.tools) && o.tools.every((t) => typeof t === "object" && t !== null && "name" in t)) {
        return { protocol: "mcp-tools-list" };
    }
    if (typeof o.agent === "object" && typeof o.capabilities === "object")
        return { protocol: "agent-cards-spec" };
    if (typeof o.tool === "object" && typeof o.safety === "object" && typeof o.audit === "object")
        return { protocol: "mcp-tool-card-spec" };
    if (typeof o.prompt === "object" && typeof o.lineage === "object")
        return { protocol: "prompt-provenance-spec" };
    if (typeof o.bundle === "object" && Array.isArray(o.items))
        return { protocol: "evidence-bundle-spec" };
    return { protocol: "unknown" };
}
function structurallyConformant(doc, protocol) {
    if (protocol === "unknown" || protocol === "otel-genai-otlp" || protocol === "mcp-tools-list")
        return { ok: true, missing: [] };
    const required = REQUIRED_BLOCKS[protocol];
    const missing = required.filter((k) => doc[k] === undefined);
    return { ok: missing.length === 0, missing };
}
function listJsonFiles(root, skip = []) {
    const out = [];
    const visit = (dir) => {
        for (const entry of readdirSync(dir)) {
            const full = join(dir, entry);
            const st = statSync(full);
            if (skip.some((s) => full.includes(s)))
                continue;
            if (st.isDirectory())
                visit(full);
...
export function run(root, opts = {}) {
    ...
    for (const file of files) {
        ...
        try {
            ...
            findings.push({ code: "malformed-json", severity: "high", message: e.message, source: file });
            continue;
        }
        const { protocol, version } = detectProtocol(parsed);
        byProtocol[protocol] += 1;
        const summary = { source: file, protocol, structurallyConformant: true };
        if (version)
        ...
        if (protocol === "unknown") {
            ...
            const { ok, missing } = structurallyConformant(parsed, protocol);
